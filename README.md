...
│
├── scripts/                           # 运行脚本
│   ├── run_all.sh / run_all.ps1       # ⭐ 总入口：单 seed 跑全部 7 类实验
│   ├── run_fed_alg_comparison.sh      # 4 联邦算法对比（FedAvg/FedProx/FedNova/SCAFFOLD）
│   ├── run_fed_alg_comparison.ps1     #   （PowerShell）
│   ├── run_fed_bcd.sh                 # BCD 联邦实验（6 划分 × FedAvg+FedProx × 多 seed）
│   ├── run_fed_bcd.ps1                #   （PowerShell）
│   ├── run_fed_torchange_bcd.sh       # torchange 联邦对比（多 seed）
│   ├── run_fed_torchange_bcd.ps1      #   （PowerShell）
│   ├── run_fed_class_comparison.sh    # Non-IID1/2/IID 类别对比（K=70，多 seed）
│   ├── run_fed_class_comparison.ps1   #   （PowerShell）
│   ├── run_centralized.sh             # 集中式训练上界
│   ├── run_centralized.ps1            #   （PowerShell）
│   ├── run_fed_fracnum_sweep.sh       # frac_num 参与率稳健性 sweep
│   ├── run_fed_fracnum_sweep.ps1      #   （PowerShell）
...
一次性运行 FedAvg 和 FedProx 在 6 种划分上的全部实验（默认 3 个 seed：42 / 2024 / 0）：

```bash
# Bash（位置参数末尾可自定义 seed 列表）
...
- **默认 3 seed**：`42 2024 0`
- **产物隔离**：每个实验的 `project_name` 内嵌 `_s<seed>` 后缀（如 `FedAvg_noniid1_K70_bcd_s42`），`results.json` 落在独立子目录，互不覆盖
...
# PowerShell
powershell -ExecutionPolicy Bypass -File scripts\run_fed_alg_comparison.ps1 -Seeds 42,2024,0
```
...
`scripts/run_all.sh`（Bash）/ `run_all.ps1`（PowerShell）是**总入口**：传一个 seed，依次调用全部 7 类实验子脚本，每个都用这同一个 seed。这是最常用的"单 seed 全量实验"工作流。
...
```bash
bash scripts/run_all.sh 42 --only centralized,alg_comparison   # 只跑这两步
bash scripts/run_all.sh 42 --skip fracnum_sweep,changen2        # 跳过这两步
...
| 步骤 | 脚本 | 内容 | 输出目录 |
|------|------|------|---------|
| 1 | `run_centralized` | 集中式上界 | `results/centralized/` |
| 2 | `run_fed_alg_comparison` | 4 联邦算法对比 | `results/alg_comparison/` |
| 3 | `run_fed_bcd` | 6 划分 × FedAvg+FedProx 主矩阵 | `results/fed_bcd/` |
| 4 | `run_fed_class_comparison` | Non-IID1/2/IID 类别异构 (K=70) | `results/class_comparison/` |
| 5 | `run_fed_torchange_bcd` | BIT-CD vs torchange 基线 | `results/torchange_fed/` |
| 6 | `run_fed_fracnum_sweep` | 参与率稳健性 sweep | `results/fracnum_sweep/` |
| 7 | `run_changen2_zeroshot` | 零样本评估（免训练） | `results/changen2_zeroshot/` |
