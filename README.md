...
- **IID 基线**：提供分层随机 IID 划分作为联邦实验的上界对比（取代集中式训练）
...
> **说明**：本项目不提供集中式（centralized）训练——性能上界由 **IID 划分下的联邦训练**（`partition_iid_K70.json` + `--iid True` 简单聚合）充当，与 Non-IID 各档同框架对比。
...
脚本默认在 `partition_dirichlet_a0.5_n7.json` 上运行；如需遍历多种异构度，取消脚本中 `PARTITIONS` 数组的注释行即可。也可直接指向 IID 划分 `partition_iid_K70.json` 作为各模型的上界对比。
...
| 实验 | 划分 | 算法 | 目的 |
|------|------|------|------|
| 联邦 IID | `partition_iid_K70.json` | FedAvg (`--iid True`) | 性能上界（取代集中式） |
...
固定 K=70，唯一变量为数据异构度（无 centralized，BCD 任务）。类 0（无变化）作为第 7 个语义类参与异构：
