"""
WHU-GCD change detection data augmentation.

Reuses the synchronous bitemporal augmentation logic from BIT_CD/datasets/data_utils.py.
All transforms are applied identically to both t1 and t2 images.
"""
...
class CDDataAugmentation:
    ...
    All random operations (flip, crop, blur) use the same random parameters
    for both t1 (im1) and t2 (im2) images to preserve spatial correspondence.
    """
    ...
    def __init__(
        ...
        self.img_size_dynamic = img_size is None
    ...
    def transform(self, imgs, labels, to_tensor=True):
        """Apply augmentation to bitemporal image pair.

        Args:
            imgs: list of ndarray [t1_img, t2_img], each HxWx3 uint8
            labels: list of ndarray [label], each HxW uint8 (0/255 for BCD)
            to_tensor: whether to convert to torch tensors

        Returns:
            (imgs, labels) — lists of tensors or PIL images
        """
        ...
        if self.with_random_crop and random.random() > 0:
            ...
            imgs = [TF.resized_crop(img, i, j, h, w, size=(self.img_size, self.img_size),
                                    interpolation=Image.CUBIC) for img in imgs]
            labels = [TF.resized_crop(img, i, j, h, w, size=(self.img_size, self.img_size),
                                      interpolation=Image.NEAREST) for img in labels]
        ...
        if self.with_scale_random_crop:
            ...
            imgs = [pil_crop(img, box, cropsize=self.img_size, default_value=0) for img in imgs]
            labels = [pil_crop(img, box, cropsize=self.img_size, default_value=255) for img in labels]
...
def pil_resize(img, size, order):
    ...
    if size[0] == img.size[0] and size[1] == img.size[1]:
        return img
    resample = Image.BICUBIC if order == 3 else Image.NEAREST
