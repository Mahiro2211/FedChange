...
# 自定义:
#   bash scripts/run_fed_class_comparison.sh 200 5 2 8 256 0.01 0.01 70 "42 2024 0"
...
for seed in "${SEEDS[@]}"; do
    ...
    for entry in "${PARTITIONS[@]}"; do
        ...
        # FedAvg (fed_alg=fedavg, fedprox_mu=0)
        proj="FedAvg_${tag}_K${K}_bcd_s${seed}"
        echo ">>> $proj"
        python -m fed_cd.federated.fed_main \
            --partition_json "$file" --project_name "$proj" \
            --fed_alg fedavg --fedprox_mu 0.0 --iid "$iid" "${COMMON[@]}"

        # FedProx (fed_alg=fedprox)
        proj="FedProx${FEDPROX_MU}_${tag}_K${K}_bcd_s${seed}"
        echo ">>> $proj"
        python -m fed_cd.federated.fed_main \
            --partition_json "$file" --project_name "$proj" \
            --fed_alg fedprox --fedprox_mu "$FEDPROX_MU" --iid "$iid" "${COMMON[@]}"
    done
done

echo ""
echo "========== Non-IID 类别对比实验完成 =========="
echo "汇总: python -m fed_cd.summarize --results_root results/class_comparison"
